...
func GetHTMLTitle(c *colly.Collector, url string) {
	...
	// product scraping
	c.OnHTML("div.sg-col-inner", func(e *colly.HTMLElement) {
		product := productInfo {}
		e.ForEach("div.rush-component", func(_ int, h *colly.HTMLElement) {
			product.Name = utils.RegexRetriever(`¥\d,`, h.DOM.Text())
			fmt.Println(utils.RegexRetriever(`¥\d*,\d*`, h.DOM.Text()))
		})
