...
func GetHTMLTitle(c *colly.Collector, url string) {
	...
	c.OnHTML("div", func(e *colly.HTMLElement) {
		name := e.DOM.Find("div").AddClass("span.a-size-base-plus a-color-base a-text-normal").Text()
		url, _ := e.DOM.Find("a").Attr("href")
		price := e.DOM.Find("span").AddClass("a-price-whole").Text()
		star := e.DOM.Find("div").AddClass("a-section a-spacing-none a-spacing-top-micro").Text()
		image := e.DOM.Find("div").AddClass("a-section aok-relative s-image-square-aspect").Find("img").Text()
		product := productInfo {
			Name: name,
			URL: url,
			Price: price,
