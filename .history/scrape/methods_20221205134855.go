...
func GetHTMLTitle(c *colly.Collector, url string) {
	...
	c.OnHTML("div", func(e *colly.HTMLElement) {
		name := utils.RegexRetriever(``, e.DOM.Text())
		URL := utils.RegexRetriever(``, e.DOM.Text())
		price := utils.RegexRetriever(`¥\d*,\d*`, e.DOM.Text())
		star := utils.RegexRetriever(``, e.DOM.Text())
		Image := utils.RegexRetriever(``, e.DOM.Text())
		product := productInfo {
			Name: ,
			URL: ,
			Price: price,
